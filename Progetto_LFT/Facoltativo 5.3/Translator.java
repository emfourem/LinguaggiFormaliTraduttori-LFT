...
public class Translator{
	...
	*/
		...
		switch(look.tag){
			...
			case Tag.ASSIGN:
             ...
             case Tag.WHILE:
                ...
                /*code.emitLabel(btrue); //etichetta a cui inizio a eseguire il corpo del while non serve se voglio cercare di eliminare un goto*/
                match(')');
              ...
              case Tag.IF:
                 ...
                 bexpr(lnext_ered_stat,case_if_false); //stampa: if_icmpR Lfalse altrimenti esegue il codice nel caso in cui la condizione sia vera
                 match(')');
                 int nuova_if=code.newLabel(); 
                 stat(nuova_if); //eseguo if con nuova etichetta altrimenti nel caso di un while annidato nell'if ci sarebbero due salti verso la stessa etichetta:uno dall'if e uno dal while
	...
	private void bexpr(int ltrue,int lfalse) {	//B -> RELOP E E
		...
		expr();
		expr();
		code.emit(opCode, lfalse); /*Non emetto l'etichetta del vero dopo l'icmpR ma quella del falso ovvero dove saltare se il contrario di ciò che voglio è vero*/
		/*code.emit(OpCode.GOto, lfalse); Non serve emetterla se il salto viene eseguito solo quando falso*/
    ...
    public static void main(String[] args) {
        ...
        String path ="if_while.lft"; // il percorso del file da leggere
        try {
